...
from _exceptions import PGPLoadError
from _targetRelease import TargetRelease


class Configuration:
    def __init__(self, config_file: str = None, pgp_privatekey_filename: str = None, arguments=None):
    ...
    def load_pgp_privatekey(self):
        process = subprocess.Popen(['gpg', '--import'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def parse_pgp_privatekey(self):
        ...
        if self.private_key_content is None or self.private_key_content == '':
            raise ValueError("No private key content supplied.")
        if (
